    ...
    */
    ...
    public function run()
    {
        ...
        foreach( new \ArrayIterator($this->validation_rules) as $var=>$opt)
        {
            ...
            {
                // is_set will set the error message, do not process further lest there are "array key does not exist" errors
                if( !$this->is_set($var) ) {
                  continue;
                }
            }
            ...
            {
              // no error message but do not process further lest there are "array key does not exist" errors
    ...
    private function validateString($var, $min=0, $max=0, $required=false)
    {
        ...
        if(isset($this->source[$var]))
        {
            ...
            {
                $this->errors[$var] = 'Field is too short';
            }
            elseif(strlen($this->source[$var]) > $max)
            {
                $this->errors[$var] = 'Field is too long';
            }
            elseif(!is_string($this->source[$var]))
            {
                $this->errors[$var] = 'Field is invalid';
            }
        }
    }
