...
class Join_Clan extends Controller {
    ...
    function get($f3, $f3_request_info = null) {
        ...
        if ($this->validate_and_clean_params($f3, 'GET', $errors, $sanitized_fields) == false) {
            ...
            $clan->load(array(
                'clan_id = ? AND status = ?',
                $sanitized_fields['clan_id'],
                \Model\Clans_List::CLAN_STATUS_ACTIVE
            ));
    ...
    function check_data_and_save($f3, &$errors, $sanitized_fields) {
        ...
        $player->load(array(
            ...
            \Model\Players_List::PLAYER_STATUS_ACTIVE
        ));
        ...
        $result = $this->sendClanJoinRequestMessage($f3, $sanitized_fields['join_player_id'], $sanitized_fields['join_clan_id'], "This is '{$player->player_name}'. Please let me join your clan '{$clan->clan_name}'.\n\n" . $f3->scrub($sanitized_fields['join_reason']));
        $invites = new \Model\Clan_Invites( $f3->get( 'DB_CLANS' ) );
		$invites->addInvite($sanitized_fields['join_player_id'], $sanitized_fields['join_clan_id']);
    ...
    private function sendClanJoinRequestMessage($f3, $fromPlayerID, $toClanID, $body) {
        ...
        $result = $message->create($fromPlayerID, \Model\Messages::MESSAGE_STATUS_ACTIVE, \Model\Messages::MESSAGE_TYPE_JOIN_CLAN_REQUEST, $body);
